...
<div class="max-w-2xl mx-auto px-4 py-8">
    ...
    <div class="mb-6 flex items-center space-x-2 text-xs text-slate-400">
        ...
        <span>&rsaquo;</span>
        <span class="text-slate-200">Review Rental #<?= $requestId ?></span>
    </div>
    ...
    <div class="card p-6 border border-slate-800 rounded-2xl bg-slate-900/60 shadow-xl space-y-6">
        <div class="flex items-center space-x-3 pb-4 border-b border-slate-800">
            <span class="p-2.5 rounded-xl bg-amber-500/10 border border-amber-500/20 text-amber-400 text-xl">⭐</span>
            <div>
                <h1 class="text-xl font-bold text-white tracking-tight">Rate & Review Your Rental Experience</h1>
                <p class="text-xs text-slate-400 mt-0.5">Your honest rating helps fellow renters and builds the owner's trust score.</p>
            </div>
        </div>
        ...
        <div class="flex items-center space-x-4 p-4 rounded-xl bg-slate-950/60 border border-slate-800">
            <?php 
                ...
                $thumb = !empty($images) ? $images[0]['image_path'] : 'assets/img/placeholder.jpg';
            ?>
            <img src="<?= base_url($thumb) ?>" alt="Product" class="w-16 h-16 rounded-xl object-cover border border-slate-800 bg-slate-900">
            <div>
                <h3 class="font-bold text-white text-sm"><?= htmlspecialchars($request->getProductTitle()) ?></h3>
                <div class="text-xs text-slate-400 mt-1">
                    <span>Rental Period: </span>
                    <span class="text-slate-300 font-medium"><?= date('M d, Y', strtotime($request->getStartDate())) ?> &rarr; <?= date('M d, Y', strtotime($request->getEndDate())) ?></span>
                </div>
            </div>
        </div>
        ...
        <?php if ($existingReview !== null): ?>
            ...
            <div class="p-5 rounded-xl bg-emerald-950/20 border border-emerald-800/40 space-y-3">
                <div class="flex items-center justify-between">
                    ...
                    <span class="text-[11px] text-slate-400"><?= date('M d, Y', strtotime($existingReview->getReviewDate())) ?></span>
                </div>
                
                <div class="text-amber-400 text-lg tracking-wider">
                    <?= str_repeat('★', $existingReview->getRating()) . str_repeat('☆', 5 - $existingReview->getRating()) ?>
                    <span class="text-xs text-slate-300 ml-2 font-mono">(<?= $existingReview->getRating() ?> / 5)</span>
                </div>

                <?php if ($existingReview->getComment()): ?>
                    <p class="text-xs text-slate-300 bg-slate-950/80 p-3 rounded-xl border border-slate-800/80 italic">
                        "<?= htmlspecialchars($existingReview->getComment()) ?>"
                    </p>
                <?php endif; ?>

                <div class="pt-2 text-right">
                    <a href="<?= base_url('renter/my_rentals.php') ?>" class="text-xs text-blue-400 hover:text-blue-300 font-semibold underline">
        ...
        <?php else: ?>
            ...
            <form method="POST" action="" class="space-y-6">
                    ...
                    <div class="flex items-center space-x-2 text-2xl" id="starContainer">
                        <?php for ($i = 1; $i <= 5; $i++): ?>
                            <label class="cursor-pointer">
                                ...
                                <span id="star-<?= $i ?>" class="transition hover:scale-110 inline-block <?= $i <= $rating ? 'text-amber-400' : 'text-slate-600' ?>">★</span>
                            </label>
                        <?php endfor; ?>
                        <span id="ratingLabel" class="text-xs text-amber-300 font-bold ml-3 font-mono"><?= $rating ?> / 5 Stars</span>
                    </div>
                </div>

                <!-- Comment Textarea -->
                <div>
                    <label for="comment" class="block text-xs font-semibold text-slate-300 mb-2">
                        Detailed Feedback & Experience (Optional)
                    </label>
                    <textarea id="comment" 
                              name="comment" 
                              rows="4" 
                              placeholder="How was the product's condition? Was the owner cooperative? Would you recommend renting this item to others?"
                              class="w-full bg-slate-950 border border-slate-800 rounded-xl px-4 py-3 text-xs text-white placeholder-slate-500 focus:outline-none focus:border-blue-500 transition"><?= htmlspecialchars($comment) ?></textarea>
                    <p class="text-[10px] text-slate-500 mt-1">Your review will be publicly visible on the product's listing page.</p>
                </div>
                ...
                <div class="pt-4 border-t border-slate-800 flex items-center justify-between">
                    ...
                    <button type="submit" 
                            class="px-6 py-2.5 bg-amber-500 hover:bg-amber-400 text-slate-950 font-bold text-xs uppercase tracking-wider rounded-xl shadow-lg shadow-amber-500/20 transition flex items-center space-x-2">
...
function updateStars(val) {
    for (let i = 1; i <= 5; i++) {
        ...
        if (i <= val) {
            ...
            star.className = "text-slate-600 transition hover:scale-110 inline-block";
        }
    }
